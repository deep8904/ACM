  ...
  postgresTest(
    ...
    async () => {
      ...
      try {
        ...
        await sql`delete from content_machine.research_packets where approved_event_id=${eventId}`;
        await sql`delete from content_machine.topic_approved_events where id=${eventId}`;
      }
    },
  );
});
