...
HTTP 429 handling is deliberately bounded. The retriever honors a valid `Retry-After` value up to the configured ceiling, otherwise uses capped exponential backoff with jitter. Migration 020 adds durable per-host attempt windows/cooldowns and negative retrieval outcomes so webhook and worker instances do not hammer the same publisher. A managed anti-bot challenge is never solved or bypassed; it immediately enters cooldown. HTTP 403 and robots denials remain blocked outcomes. Diagnostics use the non-sensitive categories `429_retry_after`, `429_cooldown`, `robots_denied`, `403_forbidden`, `alternate_official_found`, and `no_retrievable_primary`.

Official alternate discovery is suggestion-only. It accepts canonical, alternate, AMP, JSON-LD article URLs, RSS/Atom entries, and sitemap URLs only when those representations are publicly exposed and remain on the verified publisher owner domain. Every candidate must pass the same URL, DNS, robots, retrieval, extraction, and authority checks before it can be shown. An operator must still inspect and confirm a candidate; discovery never writes a source or packet. Third-party mirrors remain independent evidence.
...
A 429/403/robots recovery card offers `Retry later` when the failure is retryable, `Find another official source`, `Paste another URL`, and `Cancel source attempt`. Retry-later creates one idempotent durable job at the recorded host cooldown and has a single attempt. It cannot create a packet or duplicate the blocked research lineage; success returns to authority confirmation, and failure returns to the blocked recovery card.
